...
export async function GET(request: NextRequest) {
  ...
  const leads = await Promise.all(entreprises.map(async (ent) => {
    ...
    return {
      ...
      conseiller,
      dateTransmission: ((ent as unknown as { dateTransmission?: Date }).dateTransmission || ent.createdAt).toLocaleDateString("fr-FR"),
      dateTransmissionISO: ((ent as unknown as { dateTransmission?: Date }).dateTransmission || ent.createdAt).toISOString(),
